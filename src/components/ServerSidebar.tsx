  ...
  return (
    <TooltipProvider delayDuration={200}>
      <div className="min-w-[72px] max-w-[72px] h-screen bg-server-bg flex flex-col">
        ...
        {/* Bottom — fixed: Create & Join buttons */}
        <div className="flex flex-col items-center py-3 gap-2 shrink-0">
          <div className="w-8 h-[2px] bg-border rounded-full" />
          ...
          <Tooltip>
            ...
            <TooltipContent side="right">
              <p>{isAtLimit ? `Maksimum ${MAX_SERVERS} sunucuya ulaştınız` : 'Sunucu Oluştur'}</p>
            </TooltipContent>
          </Tooltip>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                className={`server-icon transition-colors ${
                  isAtLimit
                    ? 'bg-muted text-muted-foreground cursor-not-allowed opacity-50'
                    : 'bg-secondary text-primary hover:bg-primary hover:text-primary-foreground'
                }`}
                onClick={() => !isAtLimit && setShowJoinDialog(true)}
                disabled={isAtLimit}
              >
            ...
            <TooltipContent side="right">
              <p>{isAtLimit ? `Maksimum ${MAX_SERVERS} sunucuya ulaştınız` : 'Sunucuya Katıl'}</p>
            </TooltipContent>
          </Tooltip>
        </div>
