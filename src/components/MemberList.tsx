  ...
  const renderMember = (member: DbMember) => (
    <div key={member.id} className="flex items-center gap-2.5 px-2.5 py-2 rounded-lg hover:bg-secondary/50 cursor-pointer transition-colors">
      ...
      <div className="min-w-0">
        <p className={`text-sm truncate ${member.role === 'Bot' ? 'text-primary' : member.role === 'Admin' ? 'text-aurora-purple' : 'text-muted-foreground'}`}>{member.name}</p>
      </div>
      {member.role && (<span className="ml-auto text-[9px] bg-secondary text-muted-foreground px-1.5 py-0.5 rounded uppercase font-bold">{member.role}</span>)}
    </div>
  );
